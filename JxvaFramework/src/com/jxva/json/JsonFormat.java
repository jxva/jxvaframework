...
public class JsonFormat {
	private final static JsonFormat defaultJSON = new JsonFormat();
	...
	public JsonFormat(char quoteChar) {
		setQuoteChar(quoteChar);
	}

	public static JsonFormat defaultJSON() {
		return defaultJSON;
	}
	 ...
	 */
	private void dumpObject(StringBuilder out, Object o, char separator,
			List<String> ignoredProps) {
		if (o == null) {
			out.append("null");
		} else {
			...
			if (oClass.isPrimitive()) {
				...
				if (o instanceof DynamicProperties) {
					...
					for (String name : dynAttrs.propertyNames()) {
						...
						quote(out, name);
						out.append(':');
						dumpObject(out, dynAttrs.getProperty(name), '\0',
								ignoredProps);
					}
				}
