...
<body>
	<j:form action="book!saveBook.jv" method="post" id="save_form" bean="#request.book">
		<j:hidden name="bookId"/>
		BookName: <j:input name="name" id="name"/><br/>
		Press: <j:select name="pressId" defaultValue="#request.book.pressId">
			<j:iterate name="#request.pressList" var="list">
				<j:option value="pressId"><j:property value="pressType.name"/> - <j:property value="name"/></j:option>
			</j:iterate>
		</j:select><br/>
		Category: <j:select name="categoryId" defaultValue="#request.book.categoryId">
			<j:iterate name="#request.categoryList" var="categoryList">
				<j:option value="categoryId"><j:property value="name"/></j:option>
			</j:iterate>
		</j:select><br/>
